...
import { getDefaultRouting, clearRoutingCache, TaskType, ModelConfig } from '../../../lib/model-router';
 ...
 * Body: { task_type, model_id, temperature?, max_tokens?, top_p?, response_format?, description?, active? }
 *
...
export async function POST(req: Request) {
  try {
    ...
    const { task_type, model_id, temperature, max_tokens, top_p, response_format, description, active } = body;
    ...
    const validTaskTypes: TaskType[] = [
      'content_generation', 'deep_analysis', 'research_synthesis', 'quality_evaluation',
      'media_description', 'learning_extraction', 'format_decision', 'article_writing',
      'thread_writing', 'performance_analysis', 'shield_check', 'repo_artifact', 'casual_generation'
    ];
