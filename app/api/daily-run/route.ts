...
const ORCHESTRATOR_VERSION = GROWTH_OPERATOR_VERSION;
...
async function run(req: Request) {
  try {
    ...
    const [accountState, requirements, targets, recentContent, creatorIntel, trends] = await Promise.all([
    ...
    const rawContentPack = await generateDailyContentPack({
      ...
      creatorIntel: { creator_intel: creatorIntel.data, trends: trends.data }
    });
    ...
    const { data: runRow, error: runError } = await supabase.from('daily_checkins').upsert({
      ...
      creator_posts_analyzed: Array.isArray(creatorIntel.data) ? creatorIntel.data.length : 0,
    ...
    const sessionLog = await insertSessionLog({
      actions_completed: ['daily_run', ORCHESTRATOR_VERSION, 'x_check_attempted', 'used_research_intel', 'generated_growth_operator_pack', 'updated_daily_checkin'],
      content_created: singleTweets,
      db_updates: [{ table: 'daily_checkins', id: runRow.id }],
      pending_tasks: actions,
      next_recommendation: 'Execute growth operator pack, publish approved assets, then log URLs and metrics.'
    });

    return Response.json({ ok: true, orchestrator_version: ORCHESTRATOR_VERSION, xSnapshot, research_context: { trends: trends.data, creator_intel: creatorIntel.data }, contentPack, daily_checkin: runRow, sessionLog, pendingActions: pendingActions || [] });
  } catch (err: any) {
