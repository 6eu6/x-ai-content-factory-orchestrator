...
async function run(req: Request) {
  try {
    ...
    const { data: runRow, error: runError } = await supabase.from('daily_checkins').upsert({
      ...
      next_priority: contentPack.today_goal || 'Review generated content and publish only quality-gated items.',
      notes: `Generated by daily-run API ${ORCHESTRATOR_VERSION}. Quality statuses: ${qualityResults.map((q) => q.status).join(',')}.`
    ...
    const actions = uniqueActions(Array.isArray(contentPack.next_actions) ? contentPack.next_actions : []);
    if (actions.length) {
      await supabase.from('action_queue').insert(actions.map((instruction: string, index: number) => ({
        ...
        instruction,
        prepared_content: JSON.stringify({ ...contentPack, quality_gate: qualityResults }),
        status: 'pending',
    ...
    const sessionLog = await insertSessionLog({
      actions_completed: ['daily_run', ORCHESTRATOR_VERSION, 'x_check_attempted', 'used_research_intel', 'used_viral_memory', 'quality_gate_applied', 'updated_daily_checkin'],
      ...
      next_recommendation: 'Publish only content with publish_status=ready. Review or rewrite needs_review items before posting.'
    });
    ...
    return Response.json({
      ...
      contentPack: { ...contentPack, quality_gate: qualityResults },
